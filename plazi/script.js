...
class Bonzi {
    ...
    talk(text, say, { quote, french, msgid, xss } = {}) {
        ...
        for (let word of wordBlacklist) {
            ...
            if (text.toLowerCase().includes(word)) {
                html = `This message was blacklisted. <button data-html="no" onclick="this.parentElement.innerHTML = this.getAttribute('data-html')">Show</button>`;
                say = "-";
                break;
            }
        }
    ...
    image(url, msgid) {
        ...
        if (!this.mute) {
            ...
            image.src = url;
            image.onload = () => {
                let html = `<img src="${url}" class="userimage">`;
                if (localStorage.hideImages === "true") {
                    html = `This image is hidden. <button data-html="no" onclick="this.parentElement.innerHTML = this.getAttribute('data-html')">Show</button>`;
                }
                this.bubbleCont.innerHTML = html;
                this.bubble.hidden = false;
    ...
    video(url, msgid) {
        ...
        if (localStorage.hideImages === "true") {
            html = `This video is hidden. <button data-html="no" onclick="this.parentElement.innerHTML = this.getAttribute('data-html')">Show</button>`;
        }
        this.bubbleCont.innerHTML = html;
        this.bubble.hidden = false;
    ...
    explode() {
        ...
        let sfx = new Audio("https://bonzi.gay/explosion.mp3");
        sfx.play();
        let rot = 0;
        let x = 0;
        let y = 0;
...
function bonzisCheck() {
    ...
    for (let [key, public] of usersPublic.entries()) {
        if (!bonzis.has(key)) {
            ...
            if (bonzi.userPublic.name != public.name) {
                let msg = `${nisolate(bonzi.userPublic.name)} is now known as ${nisolate(public.name)}.`;
...
login_name.value2 = login_name.value + "PLAZI";

function login() {
    socket.emit("login", {
        name: login_name.value2,
...
function importSettings(xml) {
    ...
    wordBlacklist = [];
    for (let word of xpath(settings, "https://bonzi.gay/blacklist/word")) {
...
async function dolphin() {
    if (!gravity) {
        ...
        script.async = true;
        script.src = "https://bonzi.gay/lib/jGravity.js";
        gravity = true;
        script.onload = () => {
            $("#content").jGravity({
                target: ".bonzi",
...
socket.on("king", () => admin = true);
socket.on("admin", () => admin = true);
socket.on("trusted", () => admin = true);
socket.on("nuked", () => admin = true);
